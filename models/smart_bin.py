    ...
    def check_rfid_access(self, card_uid, weight_kg=None):
        ...
        if not card:
            ...
            _logger.warning(
                ...
                "effective_rate(before gate)=%r",
                member.name, member.id, plan.name if plan else None,
                plan.id if plan else None, plan.rate_per_kg if plan else None,
                wallet_billed_plan, wallet_billing_on,
                member.wallet_balance, low_threshold, effective_rate)
